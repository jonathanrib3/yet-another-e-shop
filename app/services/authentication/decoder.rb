  ...
  class Decoder
    ...
    def call
      ...
      if credentials.invalid?
        raise Errors::Authentication::InvalidDecodedToken, credentials.errors.to_a.join(", ")
      end
      credentials
    end
    ...
    def validate_attributes_presence
      raise Errors::Authentication::Decoder::BlankAccessToken if @access_token.blank?
    end

    def validate_decoded_user_id(user_id)
      raise Errors::Authentication::Decoder::UserNotFound unless User.exists?(user_id)
    end
