...
RSpec.describe Authentication::Issuer, type: :service do
  ...
  context 'when issuing jwt credentials, given an user' do
    ...
    it 'creates a new refresh token' do
      expect do
        auth_issuer.call
      end.to change(RefreshToken.where(crypted_token: expected_crypted_refresh_token), :count).by(1)
    end
  ...
  context 'when issuing jwt credentials, given an user, being that a refresh token has already been issued' do
    ...
    let(:expected_access_token) do
      'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOjEsImp0aSI6ImI5OGQ0MmFjLTY1MzktNDBmMy04NmU' \
      '4LWFiYzRlNjRkNDQyYSIsImlhdCI6NjEyOTIxNjAwLCJleHAiOjYxMjk2NDgwMCwiaXNzIjoibG9jYWxob3N0LnRl' \
      'c3QifQ.dhVvrCJF1mkzj5y3lxBEh-VMxS-FQv4zcQa5mLtENJc'
    end
    let(:expected_refresh_token) do
    ...
    it 'deletes the previous refresh token associated to the user' do
      expect do
        auth_issuer.call
      end.to change(RefreshToken.where({ jti: previous_refresh_token.jti }), :count).from(1).to(0)
    end

    it 'deletes the previous jti registry associated to the user' do
      expect do
        auth_issuer.call
      end.to change(JtiRegistry.where({ jti: previous_refresh_token.jti }), :count).from(1).to(0)
    end
