...
RSpec.describe ::Authenticator, type: :request do
  ...
  context 'when authenticating an user with an invalid access token' do
    ...
    context 'when an access token is in a invalid format' do
      ...
      let(:headers) do
        {
          'Authorization' => invalid_access_token.to_s
        }
      end
  ...
  context 'when authenticating an user with an access token that has a valid user but an invalid jti' do
    ...
    let!(:user) { create(:user, id: 1) }
    let(:invalid_access_token) do
      'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOjEsImp0aSI6IjVjZjBjYTA5LWM3YWMtNGY5Mi04MTU' \
      '0LTBiZDFjNWY4OTAyOCIsImlhdCI6NjEyOTMyNDAwLCJleHAiOjYxMjk3NTYwMCwiaXNzIjoibG9jYWxob3N0LnRl' \
      'c3QifQ.nG5zL137NQmtH7dmzowflj6DostVKNVD3AJ3A8JMTIE'
    end
